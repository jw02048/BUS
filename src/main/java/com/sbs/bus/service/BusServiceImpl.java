...
public class BusServiceImpl implements BusService {
	...
	public Map<String, Object> doReserve(Map<String, Object> param) {
		...
		// 2.ticketCode 생성
		// 3.DB 티켓 INSERT VALUE (ticketCode*, departureDate,serviceId,seatNum,email,charge)
		// 4.이메일로 티켓 정보 전송
		...
		try {
			...
			mail.setText(new StringBuffer().append("<h1>예매 정보</h1>")
					.append("티켓코드 : " + ticketCode + "<br>")
					...
					.append("좌석번호 : " + param.get("seatNum") + "<br>")
					.append("결제자 : " + ticketInfo.getExtra().get("memberName") + "<br>")
					.append("결제금액 : " + param.get("charge") +"원 <br>").toString());
			mail.send();
			msg = "예매 되었습니다.";
			resultCode = "S-2";
		} catch (Exception e) {
		...
		return Maps.of("msg", msg, "resultCode", resultCode, "ticketId", param.get("id"));
	}
